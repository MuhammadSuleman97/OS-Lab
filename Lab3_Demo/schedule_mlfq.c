...
int main()
{
     int i,j,k=0,r=0,time=0,tq1=10,tq2=20,flag=0;
     char c;
...
sortByArrival();
time=Q1[0].AT;
printf("Process in first queue following RR with qt=10");
printf("\nProcess\t\tRT\t\tWT\t\tTAT\t\t");
for(i=0;i<n;i++)
{
...
if(flag==1)
 {printf("\nProcess in second queue following RR with qt=20");
  printf("\nProcess\t\tRT\t\tWT\t\tTAT\t\t");
}for(i=0;i<k;i++)
   {
    if(Q2[i].RT<=tq2)
...
{if(flag==2)
printf("\nProcess in third queue following RR with qt = 30 ");
}
for(i=0;i<r;i++)
{
    if(i==0)
